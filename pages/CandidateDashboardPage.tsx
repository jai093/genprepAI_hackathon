...
import { UploadCloud, CheckCircle, AlertTriangle, Lightbulb, TrendingUp, BarChart, History, Award, BookOpen, Mic, XCircle } from 'lucide-react';
        ...
        <div className="space-y-6">
            {selectedReport && (
                <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4 animate-fade-in-fast">
                    <div className="bg-slate-50 rounded-xl shadow-2xl w-full max-w-5xl h-full max-h-[95vh] overflow-y-auto relative">
                        <button 
                        ...
                        <InterviewReport session={selectedReport} onRestart={() => setSelectedReport(null)} backPath="" showChat={true} />
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            {interviewHistory.slice(0, 3).map((session, index) => (
                                <div key={index} className="flex justify-between items-center p-3 bg-slate-50 rounded-lg">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <button onClick={() => setSelectedReport(session)} className="text-sm text-indigo-600 hover:underline">View Report</button>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-slate-200">
                        <h3 className="text-lg font-semibold flex items-center mb-4"><Award size={20} className="mr-2 text-indigo-500"/>AI-Powered Tips</h3>
                         <ul className="space-y-3 text-sm text-slate-600">
                            <li className="flex items-start"><span className="text-indigo-500 font-bold mr-2">1</span><span>Try using the STAR method (Situation, Task, Action, Result) for behavioral questions.</span></li>
                            <li className="flex items-start"><span className="text-indigo-500 font-bold mr-2">2</span><span>Practice concise answers that stay on topic and directly address the question.</span></li>
                            <li className="flex items-start"><span className="text-indigo-500 font-bold mr-2">3</span><span>Review your past interviews to identify patterns and areas for improvement.</span></li>
                         </ul>
                    </div>
                    <div className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-slate-200">
            ...
            <div className="max-w-2xl mx-auto">
                <div className="bg-white p-6 sm:p-8 rounded-xl shadow-sm border border-slate-200">
                    ...
                    {!import.meta.env.VITE_GEMINI_API_KEY && (
                        ...
                        <span>
                            <strong>Warning:</strong> An API key for the AI service is not configured. Please set up your <code>VITE_GEMINI_API_KEY</code> in a <code>.env.local</code> file. The features on this page will not work.
                        </span>
                        </div>
                    )}
                </div>

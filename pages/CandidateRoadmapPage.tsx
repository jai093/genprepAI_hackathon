...
const CandidateRoadmapPage: React.FC = () => {
  ...
  const fetchRoadmap = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await generateRoadmap(resumeData.skills, targetRole || 'Software Engineer');
  ...
  }, [resumeData, targetRole, setCareerRoadmap, setLoading, setError]);
  
  useEffect(() => {
    if (resumeData && !careerRoadmap) {
        fetchRoadmap();
    }
  }, [resumeData, careerRoadmap, fetchRoadmap]);
      ...
      <div className="flex flex-wrap items-center gap-4 p-4 bg-white rounded-xl shadow-sm border border-slate-200">
       ...
       {!import.meta.env.VITE_GEMINI_API_KEY && (
        <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-yellow-800 text-sm flex items-start">
            ...
            <span><strong>Warning:</strong> An API key is not configured. Please set up your <code>VITE_GEMINI_API_KEY</code>. This feature is disabled.</span>
        </div>
        )}

      <div className="relative">
        {isLoading && careerRoadmap && (
            <div className="absolute inset-0 bg-white/70 backdrop-blur-sm z-10 flex flex-col items-center justify-center rounded-xl animate-fade-in-fast">
                <Spinner />
                <p className="mt-2 font-semibold text-slate-600">Generating new roadmap...</p>
            </div>
        )}
        {careerRoadmap && (

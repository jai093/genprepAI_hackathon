...
import type { Assessment, InterviewFeedback, InterviewSession, InterviewSummary, TranscriptEntry, InterviewConfig } from '../types';
...
type InterviewSubStage = 'asking' | 'listening' | 'analyzing' | 'transitioning' | 'generating_summary' | 'finished';
...
const InterviewUI: React.FC<{
    ...
    const handleAnswerSubmission = useCallback(async (finalTranscript: string) => {
        ...
        const currentQuestion = assessment.questions[currentQuestionIndexRef.current];
        try {
            const fb = await getInterviewFeedback(currentQuestion, answer);
            setSessionTranscript(prev => [...prev, { question: currentQuestion, answer, feedback: fb }]);
        } catch (err) {
            ...
            setSessionTranscript(prev => [...prev, { question: currentQuestion, answer, feedback: emptyFeedback }]);
        } finally {
            setInterviewStage('transitioning');
        }
    }, [assessment.questions, setSessionTranscript, setError]);
    ...
    const handleFinishInterview = useCallback(async () => {
        ...
        if (sessionTranscript.length === 0) {
            ...
            const dummySession: InterviewSession = {
                ...
                summary: { overallSummary: 'Interview not completed.', actionableTips: [], encouragement: '', simulatedFacialExpressionAnalysis: 'N/A', simulatedBodyLanguageAnalysis: 'N/A', simulatedAudioAnalysis: 'N/A' }
            };
    ...
    }, [sessionTranscript, assessment, sessionDuration, onComplete, setError]);
    ...
    useEffect(() => {
        ...
        recognition.onerror = (event) => {
            ...
            if (event.error === 'no-speech') {
                userMessage = "I didn't catch that. Please ensure your microphone is working.";
    ...
    }, [stopTimer, handleAnswerSubmission, setError]);
